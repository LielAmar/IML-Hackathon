...
# ===== Testing & Plotting =====
def create_estimators_comparison_graph(df):
    print("Running estimator tester...")

    X, y, X_test, y_test = split_data(df, include_dev=False)
    ...
    for name, model in models.items():
        ...
        scores[name].append(f1_score(y_pred, ~y_test.isna(), average="macro"))
        scores[name].append(f1_score(y_pred, ~y_test.isna()))
        scores[name].append(accuracy_score(y_pred, ~y_test.isna()))
        scores[name].append(mean_squared_error(y_pred.astype(int), ~y_test.isna().astype(int)))
...
def create_train_vs_test_graph(df):
    X, y, X_test, y_test = split_data(df, include_dev=False)
    ...
    X_dev_top_important = X_test[["cancellation_policy_21", "cancellation_policy_30", "no_of_adults", "time_ahead", "cancellation_policy_14"]]
    ...
    for index, percentage in enumerate(percentages):
        ...
        top_important_scores_test[index-1] = f1_score(model.predict(X_dev_top_important), ~y_test.isna(), average="macro")
