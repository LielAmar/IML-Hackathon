...
def fit_over_dataset():
    ...
    joblib.dump(model, 'ridge5.5.joblib', compress=9)


def run_task_2(df, output_file):
    model = joblib.load("./hackathon_code/ridge5.5.joblib")
...
def create_ridge_lasso_graph(df):
    X_train, X_dev, X_test, y_train, y_dev, y_test = split_data(df)
